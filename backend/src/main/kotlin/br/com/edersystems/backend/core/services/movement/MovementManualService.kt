...
class MovementManualService(private val repository: IMovimentoManualRepository) : AbstractService() {
	...
	fun validateMonth(month: BigDecimal): ExceptionError? {
		return if (month.isInvalid()) {
			createError(MessageCodeUtil.PROD_0005)
		} else null
	}

	fun validateYear(year: BigDecimal): ExceptionError? {
		return if (year.isInvalid()) {
			createError(MessageCodeUtil.PROD_0006)
		} else null
	}

	fun validateNumberLaunch(numberLaunch: BigDecimal): ExceptionError? {
		return if (numberLaunch.isInvalid()) {
			createError(MessageCodeUtil.PROD_0008)
		} else null
	}

	fun validateCodProduto(codProduto: UUID): ExceptionError? {
		return if (Objects.isNull(codProduto)) {
			createError(MessageCodeUtil.PROD_0003)
		} else null
	}

	fun validateCodCosif(codCosif: UUID): ExceptionError? {
		return if (Objects.isNull(codCosif)) {
			createError(MessageCodeUtil.PROD_0007)
		} else null
	}
	...
	fun validateUserCode(userCode: String): ExceptionError? {
		return if (userCode.isBlank()) {
			createError(MessageCodeUtil.PROD_0009)
		} else null
	}

	fun validateAmount(amount: BigDecimal): ExceptionError? {
		return if (amount.isInvalid()) {
			createError(MessageCodeUtil.PROD_0010)
		} else null
	}
